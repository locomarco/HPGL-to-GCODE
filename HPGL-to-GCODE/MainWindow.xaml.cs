...
namespace HPGL_to_GCODE
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private void ReadHpglFile(string filename)
        {
            ...
            HPGL hpgl = HPGL.Parse(reader.ReadToEnd());
            ProcessHPGL(hpgl);
            UpdateLabels(sizeX, sizeY);
        }

        private void ProcessHPGL(HPGL hpgl)
        {
            Profile p = null;
            ...
            foreach (var command in _hpgl.Commands)
            {
                ...
                {
                    float x = (float)command.Coordinates[i].X * resolution + xOffset;
                    float y = (float)command.Coordinates[i].Y * resolution + yOffset;

                    GCode.AppendLine($"G1 X{x} Y{y}");

                    sizeX = (float)Math.Max(sizeX, command.Coordinates[i].X);
                    sizeY = (float)Math.Max(sizeY, command.Coordinates[i].Y);
                }
            }
        ...
        private void UpdateLabels(float sizeX, float sizeY)
        {
